...
mdef := 'include_top = False, weights = "imagenet"';
STRING modName := 'ResNet50';
s := GNNI.GetSession(1);
resnet_model := GNNI.DefineKAModel(s, modName, mdef);
OUTPUT(resnet_model, NAMED('resnet_model'));

resnet_model_summary := GNNI.getSummary(resnet_model);

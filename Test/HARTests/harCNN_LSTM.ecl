...
/**
  ...
  * This test specifically evaluates the gestures using an LSTM Recurrent Neural Network front ended by a
  * Convolutional Network.  This combination looks for patterns regardless of where they fall in the time-
  * series, and then uses LSTM to evaluate the time-series pattern detected.  This typically yields better
  * results than LSTM or CNN alone.
...
// formed.
// For the CNN input layer, we split the 8 timeperiods into 4 sets of length 2.
// Thus our Tensor shape is [0, 4, 2, 9].  We do this using Tensor.Reshape since it is easier than calculating
// The indexes ourselves.
...
trainX1 := Tensor.R4.MakeTensor([0, 8, 9], trainX0);
// Dimensions for the dependent (Y) data are [numRecords, numClasses]
trainY:= Tensor.R4.MakeTensor([0, 6], trainY0);
// Use Reshape to convert X to the four dimensional input needed by the CNN layer.
trainX := Tensor.R4.Reshape(trainX1, [0, 4, 2, 9]);
...
testX1 := Tensor.R4.MakeTensor([0, 8, 9], testX0);
testY := Tensor.R4.MakeTensor([0, 6], testY0);

// Use Reshape to convert  test X to the four dimensional input needed by the CNN layer.
testX := Tensor.R4.Reshape(testX1, [0, 4, 2, 9]);
...
ldef := ['''layers.TimeDistributed(layers.Conv1D(filters=64, kernel_size=1, activation='relu'), input_shape=(4,2,9))''',
        '''layers.TimeDistributed(layers.Conv1D(filters=64, kernel_size=1, activation='relu'))''',

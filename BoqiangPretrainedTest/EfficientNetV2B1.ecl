/*
About this test:
  Test the usability of Pre-trained Model EfficientNetV2B1.
  Reference: https://www.tensorflow.org/api_docs/python/tf/keras/applications/efficientnet_v2/EfficientNetV2B1
  Input shape = (240, 240, 3) 

Results:

class                   probability
tusker	                0.5328111052513123
African_elephant	      0.3798011541366577
Indian_elephant	        0.008343950845301151
*/
...
SET OF INTEGER hexToNparry(DATA byte_array):= EMBED(Python)
  ...
  I_array = tf.keras.applications.efficientnet_v2.preprocess_input(I_array)
...
x3 := PROJECT(x2, TRANSFORM(TensData, SELF.indexes := [1, TRUNCATE(LEFT.id/(240*3)) + 1, TRUNCATE(LEFT.id/3)%240 + 1, LEFT.id%3 + 1], SELF.value := LEFT.value));
x := Tensor.R4.MakeTensor([0,240,240,3], x3);

// load the model
s := GNNI.GetSession(1);
ldef := ['''applications.efficientnet_v2.EfficientNetV2B1(weights = "imagenet")'''];

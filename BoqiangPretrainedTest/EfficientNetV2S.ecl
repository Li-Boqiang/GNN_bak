/*
About this test:
  Test the usability of Pre-trained Model EfficientNetV2S.
  Reference: https://www.tensorflow.org/api_docs/python/tf/keras/applications/efficientnet_v2/EfficientNetV2S
  Input shape = (384, 384, 3) 
  
Results:

class                   probability
tusker	                0.7968520522117615
African_elephant	      0.1137961819767952
Indian_elephant	        0.002234777435660362
*/
...
x3 := PROJECT(x2, TRANSFORM(TensData, SELF.indexes := [1, TRUNCATE(LEFT.id/(384*3)) + 1, TRUNCATE(LEFT.id/3)%384 + 1, LEFT.id%3 + 1], SELF.value := LEFT.value));
x := Tensor.R4.MakeTensor([0,384,384,3], x3);

// load the model
s := GNNI.GetSession(1);
ldef := ['''applications.efficientnet_v2.EfficientNetV2S(weights = "imagenet")'''];

/*
About this test:
  Test the usability of Pre-trained Model EfficientNetV2B2.
  Reference: https://www.tensorflow.org/api_docs/python/tf/keras/applications/efficientnet_v2/EfficientNetV2B2
  Input shape = (260, 260, 3) 

Results:

class                   probability
tusker	                0.592946469783783
African_elephant	      0.3367547690868378
Indian_elephant	        0.0141741530969739
*/
...
x3 := PROJECT(x2, TRANSFORM(TensData, SELF.indexes := [1, TRUNCATE(LEFT.id/(260*3)) + 1, TRUNCATE(LEFT.id/3)%260 + 1, LEFT.id%3 + 1], SELF.value := LEFT.value));
x := Tensor.R4.MakeTensor([0,260,260,3], x3);

// load the model
s := GNNI.GetSession(1);
ldef := ['''applications.efficientnet_v2.EfficientNetV2B2(weights = "imagenet")'''];

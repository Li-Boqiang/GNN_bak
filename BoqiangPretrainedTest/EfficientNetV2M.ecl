/*
About this test:
  Test the usability of Pre-trained Model EfficientNetV2M.
  Reference: https://www.tensorflow.org/api_docs/python/tf/keras/applications/efficientnet_v2/EfficientNetV2M
  Input shape = (480, 480, 3) 

Results:

class                   probability
tusker	                0.6466392278671265
African_elephant	      0.1872197091579437
Indian_elephant	        0.006474968977272511
*/
...
x3 := PROJECT(x2, TRANSFORM(TensData, SELF.indexes := [1, TRUNCATE(LEFT.id/(480*3)) + 1, TRUNCATE(LEFT.id/3)%480 + 1, LEFT.id%3 + 1], SELF.value := LEFT.value));
x := Tensor.R4.MakeTensor([0,480,480,3], x3);

// load the model
s := GNNI.GetSession(1);
ldef := ['''applications.efficientnet_v2.EfficientNetV2M(weights = "imagenet")'''];

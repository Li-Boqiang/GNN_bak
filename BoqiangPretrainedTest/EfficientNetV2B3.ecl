/*
About this test:
  Test the usability of Pre-trained Model EfficientNetV2B3.
  Reference: https://www.tensorflow.org/api_docs/python/tf/keras/applications/efficientnet_v2/EfficientNetV2B3
  Input shape = (300, 300, 3) 

Results:

class                   probability
tusker	                0.68971848487854
African_elephant	      0.2295800447463989
Indian_elephant	        0.005837815348058939
*/
...
x3 := PROJECT(x2, TRANSFORM(TensData, SELF.indexes := [1, TRUNCATE(LEFT.id/(300*3)) + 1, TRUNCATE(LEFT.id/3)%300 + 1, LEFT.id%3 + 1], SELF.value := LEFT.value));
x := Tensor.R4.MakeTensor([0,300,300,3], x3);

// load the model
s := GNNI.GetSession(1);
ldef := ['''applications.efficientnet_v2.EfficientNetV2B3(weights = "imagenet")'''];

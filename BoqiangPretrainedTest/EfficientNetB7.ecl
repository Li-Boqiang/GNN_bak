/*
About this test:
  Test the usability of Pre-trained Model EfficientNetB7.
  Reference: https://www.tensorflow.org/api_docs/python/tf/keras/applications/efficientnet/EfficientNetB7
  Input shape = (600, 600, 3) 

Results:

class                   probability
tusker	                0.5968149900436401
African_elephant	      0.2510651350021362
Indian_elephant	        0.001907072262838483
*/
...
x3 := PROJECT(x2, TRANSFORM(TensData, SELF.indexes := [1, TRUNCATE(LEFT.id/(600*3)) + 1, TRUNCATE(LEFT.id/3)%600 + 1, LEFT.id%3 + 1], SELF.value := LEFT.value));
x := Tensor.R4.MakeTensor([0,600,600,3], x3);

// load the model
s := GNNI.GetSession(1);
ldef := ['''applications.efficientnet.EfficientNetB7(weights = "imagenet")'''];
